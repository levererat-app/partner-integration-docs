...
| `payment_method`         |                   yes                    | Payments methods starting with `levererat` is our own payment solutions. If you already have a payment solution, you could use any of the `company_` payment methods. For testing, please use `test` . The order is instantly deleted after creation | `levererat_swish` `company_swish` `company_invoice` `company_card` `test` |                -                 |
